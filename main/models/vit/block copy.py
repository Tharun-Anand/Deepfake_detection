...
#         self.attn_map = None #added to visualize the attention maps
...
#         self.attn_map = attn  # Store the attention map here
#         #print(self.attn_map.shape,'attn_map')
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # Use gamma_1 and gamma_2 if init_values is provided
        if init_values > 0:
            self.gamma_1 = nn.Parameter(init_values * torch.ones((dim)), requires_grad=True)
            self.gamma_2 = nn.Parameter(init_values * torch.ones((dim)), requires_grad=True)
        else:
    ...
    def forward(self, x1, x2=None):
        ...
        if x2 is None:
            ...
            if self.gamma_1 is None:
                ...
                # Apply gamma scaling if init_values is provided
                x1 = x1 + (self.gamma_1 * self.attn(self.norm1(x1)))
                x1 = x1 + (self.gamma_2 * self.mlp(self.norm2(x1)))
        else:
            # Cross-attention
            if self.gamma_1 is None:
                # Standard cross-attention path
                x1 = x1 + self.attn(self.norm1(x1), self.norm1(x2))  # Pass both x1 and x2 to attn
                x1 = x1 + self.mlp(self.norm2(x1))
            else:
                # Apply gamma scaling if init_values is provided
                x1 = x1 + (self.gamma_1 * self.attn(self.norm1(x1), self.norm1(x2)))  # Pass both x1 and x2
                x1 = x1 + (self.gamma_2 * self.mlp(self.norm2(x1)))
