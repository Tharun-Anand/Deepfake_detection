...
class ViTDecoder(ViTBase):
    def __init__(self, audio_num_patches, video_num_patches,
        ...
        self.audio_proj = nn.Linear(embed_dim, audio_target_embed_dim)
    ...
    def init_positional_embedding(self, num_patches, embed_dim, dropout_rate=0):
        self.audio_pos_embedding = SinCosPositionalEmbedding((self.audio_num_patches, embed_dim), dropout_rate=dropout_rate)
    ...
    def forward(self,video_embeddings):
        ...
        #audio_embeddings = self.audio_pos_embedding(audio_embeddings)
        video_embeddings = self.video_pos_embedding(video_embeddings)

        #audio_embeddings = self.backbone(audio_embeddings)
        video_embeddings = self.backbone(video_embeddings)

        #audio_embeddings = torch.sigmoid(self.audio_proj(audio_embeddings))
        #video_embeddings = torch.sigmoid(self.video_proj(video_embeddings))
