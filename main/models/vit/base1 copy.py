...
class VitBackBone(nn.Module):
    ...
    # def log_weights(self, log_file="weights_log2.txt"):
    #     with open(log_file, "w") as f:
    #         f.write("Weights in nn.ModuleList:\n")
    #         for i, block in enumerate(self.blocks):
    #             for name, param in block.named_parameters():
    #                 f.write(f"Block {i} - {name}: {param.mean().item()}\n")
        
        #self.blocks.load_state_dict(torch.load('/home/sushanth/deepfake_detection/tharun/initialization_weights/backbone_weights/video_encoder_backbone1.pt'))
    ...
    def forward(self, x):
        ...
        # print("Weights in nn.ModuleList:")
        # for i, block in enumerate(self.blocks):
        #     for name, param in block.named_parameters():
        #         print(f"Block {i} - {name}: {param.mean()}")
        for i, block in enumerate(self.blocks):
...
class ViTBase(nn.Module):
    ...
    def init_positional_embedding(self,num_patches, embed_dim, dropout_rate=0.):
        ...
        #self.pos_embedding1.load_state_dict(torch.load('/home/sushanth/deepfake_detection/tharun/initialization_weights/positional_weights/video_encoder.pos_embedding1.pt'))
        
        self.pos_embedding2 = SinCosPositionalEmbedding((num_patches, embed_dim), dropout_rate=dropout_rate)
        # self.pos_embedding2.load_state_dict(torch.load('/home/sushanth/deepfake_detection/tharun/initialization_weights/positional_weights/video_encoder.pos_embedding2.pt'))
        # for param in self.pos_embedding2.parameters():
    ...
    def init_backbone(self, embed_dim=768, depth=12, num_heads=12,
        ...
        #self.backbone1.load_state_dict(torch.load('/home/sushanth/deepfake_detection/tharun/initialization_weights/backbone_weights/video_encoder_backbone1.pt'))
        #self.backbone2.load_state_dict(torch.load('/home/sushanth/deepfake_detection/tharun/initialization_weights/backbone_weights/video_encoder_backbone2.pt'))
